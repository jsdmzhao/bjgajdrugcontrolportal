package com.unis.app.car.model;

public class Car {

	private String n_xh;
	
	private String c_cphm;
	
	private String c_yhzid;
	
	private String d_dj;
	
	private String c_yhid;
	
	private String n_cllbxh;
	
	private String c_sqsy;
	
	private String d_ccsj;
	
	private String d_gdsj;
	
	private String c_sfdxsq;
	
	private String n_sfsh;
	
	private String c_shr;
	
	private String c_shjg;
	
	private String d_shsj;
	
	
	public String getN_xh() {
		return n_xh;
	}

	public void setN_xh(String nXh) {
		n_xh = nXh;
	}

	public String getC_cphm() {
		return c_cphm;
	}

	public void setC_cphm(String cCphm) {
		c_cphm = cCphm;
	}

	public String getC_yhzid() {
		return c_yhzid;
	}

	public void setC_yhzid(String cYhzid) {
		c_yhzid = cYhzid;
	}

	public String getD_dj() {
		return d_dj;
	}

	public void setD_dj(String dDj) {
		d_dj = dDj;
	}

	public String getC_yhid() {
		return c_yhid;
	}

	public void setC_yhid(String cYhid) {
		c_yhid = cYhid;
	}

	public String getN_cllbxh() {
		return n_cllbxh;
	}

	public void setN_cllbxh(String nCllbxh) {
		n_cllbxh = nCllbxh;
	}

	public String getC_sqsy() {
		return c_sqsy;
	}

	public void setC_sqsy(String cSqsy) {
		c_sqsy = cSqsy;
	}

	public String getD_ccsj() {
		return d_ccsj;
	}

	public void setD_ccsj(String dCcsj) {
		d_ccsj = dCcsj;
	}

	public String getD_gdsj() {
		return d_gdsj;
	}

	public void setD_gdsj(String dGdsj) {
		d_gdsj = dGdsj;
	}

	public String getC_sfdxsq() {
		return c_sfdxsq;
	}

	public void setC_sfdxsq(String cSfdxsq) {
		c_sfdxsq = cSfdxsq;
	}

	public String getN_sfsh() {
		return n_sfsh;
	}

	public void setN_sfsh(String nSfsh) {
		n_sfsh = nSfsh;
	}

	public String getC_shr() {
		return c_shr;
	}

	public void setC_shr(String cShr) {
		c_shr = cShr;
	}

	public String getC_shjg() {
		return c_shjg;
	}

	public void setC_shjg(String cShjg) {
		c_shjg = cShjg;
	}

	public String getD_shsj() {
		return d_shsj;
	}

	public void setD_shsj(String dShsj) {
		d_shsj = dShsj;
	}

}
