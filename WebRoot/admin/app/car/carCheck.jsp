...
<head>
    ...
    <script src='<%=basePath%>dwr/engine.js' type='text/javascript' ></script>
  	...
  	<script src='<%=basePath%>dwr/interface/CarCheckAction.js' type='text/javascript' ></script>
...
<body style="padding:10px;height:100%; text-align:center;">
  ...
  <script type="text/javascript">
      ...
      var grid = $("#maingrid").ligerGrid({
          ...
          { display: "检测日期", name: "d_jcrq", width: 160, type: "text", align: "left", editor: { type: 'text'}},
          { display: "检测厂名称", name: "c_jccmc", width: 200, type: "text", align: "left", editor: { type: 'text'}},
          { display: "车辆行驶总里程", name: "n_clxszlc", width: 120, type: "text", align: "left", editor: { type: 'text'}}
          ], dataAction: 'server', pageSize: 20, toolbar: {},sortName: 'n_xh', url:'<%=basePath%>carCheckPageList',
      ...
      function toolbarBtnItemClick(item) {
          ...
          switch (item.id) {
              case "add":
                  dialog = $.ligerDialog.open({ url: '<%=basePath%>admin/app/car/carCheckDetail.jsp', 
                  		  title: '车辆检测信息',
              ...
              case "view":
                  ...
                  dialog = $.ligerDialog.open({ url: '<%=basePath%>carCheckView?carCheck.n_xh=' + selected.n_xh,
              		  title: '车辆检测信息',	 
                      height: 450,width: 900,showMax: true, showToggle: true,  showMin: true
			  });
                  break;
              case "modify":
                  var selected = grid.getSelected();
                  if (!selected) { LG.tip('请选择行!'); return }
                  dialog = $.ligerDialog.open({ url: '<%=basePath%>carCheckUpdate?carCheck.n_xh=' + selected.n_xh,
                  		  title: '车辆检测信息',	 
                          height: 450,width: 900,showMax: true, showToggle: true,  showMin: true
				  });
                  break;
      ...
      function f_delete() {
          var selected = grid.getSelected();
          if (selected) {
        	  CarCheckAction.carCheckDelete(selected.n_xh, function (result){
             	   if(result == 'success'){
             		  LG.showSuccess('删除成功');
      ...
      grid.bind('afterSubmitEdit', function (e) {
          ...
          CarCheckAction.carCheckSave(data, function (result){
        	if(result == 'success'){
        		LG.showSuccess('保存成功');
      		} else {
      			LG.showSuccess('保存失败');
