          ...
          switch (item.id) {
              case "add":
            	//  f_dialog("add","新增上下班信息");
            	   dialog = $.ligerDialog.open({ title :'新增信息',url: '<%=basePath%>admin/app/duty/kqqqsbDetail.jsp', 
            ...
            //      top.f_addTab(null, '查看上下班信息', '<%=basePath%>admin/app/duty/kqqqsbDetail.jsp?IsView=1&ID=' + selected.UserID);
            //      break;
              case "modify":
            	  
            	  var selected = grid.getSelected();
                        if (!selected) { LG.tip('请选择行!'); return }
                       dialog = $.ligerDialog.open({ title :'修改信息',url: '<%=basePath%>admin/app/duty/kqqqsbDetail.jsp?nXh=' + selected.nXh, 
                       height: 350,width: 720,showMax: true, showToggle: true,  showMin: true
				  });
                       break;
