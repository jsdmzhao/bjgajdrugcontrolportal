...
<script type='text/javascript'
	src='<%=basePath%>dwr/interface/KqQqsbSvc.js'></script>
<script type='text/javascript'
 	...
 	LG.appendSearchButtons("#formsearch", grid);
      ...
      LG.loadToolbar(grid, toolbarBtnItemClick);
    	...
    	function loadGrid(obj){
    		if(!obj)obj={};
    		KqQqsbSvc.queryByPage(obj,oPage,function(rdata){
    			if(rdata == null){
     ...
     //     alert($d(grid.data ));
      ...
      function toolbarBtnItemClick(item) {
          switch (item.id) {
              case "add":
            	//  f_dialog("add","新增上下班信息");
            	   dialog = $.ligerDialog.open({ title :'新增信息',url: '<%=basePath%>admin/app/duty/kqqqsbDetail.jsp?a=1<%=str%>', 
            ...
            //      top.f_addTab(null, '查看上下班信息', '<%=basePath%>admin/app/duty/kqqqsbDetail.jsp?IsView=1&ID=' + selected.UserID+'<%=str%>');
            //      break;
              case "modify":
            	  
            	  var selected = grid.getSelected();
                        if (!selected) { LG.tip('请选择行!'); return }
                       dialog = $.ligerDialog.open({ title :'修改信息',url: '<%=basePath%>admin/app/duty/kqqqsbDetail.jsp?nXh=' + selected.nXh+'<%=str%>', 
                       height: 350,width: 720,showMax: true, showToggle: true,  showMin: true
				  });
                       break;
		...
		function f_remove() {
			var selected = grid.getSelected();
			if (selected) {
				KqQqsbSvc.remove(selected, function(rdata) {
					if (rdata) {
						LG.showSuccess('删除成功');
						loadGrid();
					} else {
